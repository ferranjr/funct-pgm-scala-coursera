...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = ((w groupBy( c => c.toLower)) map ( l => ( l._1 , l._2.length ) ) toList) sorted

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = wordOccurrences((s flatMap( ws => ws.map( c => c))).mkString)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    ...
    case head :: tail =>
		...
		combs ::: (for ( comb <- combs; i <- 1 to head._2) yield ( head._1 , i ) :: comb )
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = 
    (y.toMap.foldLeft( x.toMap )( ( map, tuple ) => {
    	val freq = map(tuple._1) - tuple._2
    	if(freq == 0 ) map - tuple._1
    	else map.updated(tuple._1, freq)
    })).toList.sorted
